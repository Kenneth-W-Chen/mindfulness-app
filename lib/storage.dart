  ...
  Future<Map<ActivityName, Map<String, Object?>>> getActivityLogs(
    ...
    for (var activity in activities) {
      var row = (await _db.rawQuery(
          ...
          'WHERE'
          'name = ?',
          [_activityTable, _activityLogTable, activity.name]))[0];
      logs[activity] = {
        'completion_date':
            (row['completion_date'] as int).epochDaysToDateTime(),
        'info': row['info']
...
void main() async {
  ...
  await NotificationService
      .initLocalNotifications(); // Initialize local notifications
  await NotificationService
      .requestNotificationPermissions(); // Request notification permissions
  await NotificationService.getFCMToken(); // Get FCM token
  runApp(MyApp());
}
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        ...
        body: Center(
          child: Text('Welcome to the Mindfulness App'),
        ),
      ),
    );
  }
